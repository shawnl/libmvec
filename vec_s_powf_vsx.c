...
static inline vector float exp2_inline(vector double xdl, vector double xdr, vector unsigned sign_bias)
{
	...
	sign_biasl = (v64u)vec_mergeh(zero, sign_bias);
	sign_biasr = (v64u)vec_mergel(zero, sign_bias);
#if TOINT_INTRINSICS
...
static inline vector unsigned checkint(vector unsigned iy)
{
	...
	vector unsigned res;
	vector unsigned csts = {0, 1, 2, 0x7f};
	vector unsigned first_cst = vec_splat(csts, 3);
	vector unsigned is_first = (vector unsigned) vec_cmplt (e, first_cst);
	not_matched &= ~is_first;
	res = zero;
	vector unsigned is_second = (vector unsigned) vec_cmpgt (e, first_cst + 23);
	not_matched &= ~is_second;
	res = vec_sel(res, vec_splat(csts, 2), is_second);
	...
	res = vec_sel(res, vec_splat(csts, 1), is_four & not_matched);
	not_matched &= ~is_four;
	res = vec_sel(res, vec_splat(csts, 2), not_matched);
	return res;
}
...
zeroinfnan(vector unsigned ix)
{
	vector unsigned cst
		= {2u * 0x7f800000 - 1, 2u * 0x7f800000 - 1,
		   2u * 0x7f800000 - 1, 2u * 0x7f800000 - 1};
	return (vector unsigned)vec_cmpge(2 * ix - 1, cst);
}
...
//	if (__glibc_unlikely
	...
	if (!vec_all_eq(is_special | is_zeroinfnanx, zero)) {
		if (!vec_all_eq(is_zeroinfnanx, zero)) {
			...
			vector unsigned consts = {0x3f800000, 2u * 0x7f800000, 2u * 0x7f800000, 2 * 0x3f800000};
			vector unsigned is_two = (vector unsigned) vec_cmpeq (ix, vec_splat(consts, 0));
			res = vec_sel(res, vasuint(one), (is_one | is_two) & not_covered);
			not_covered &= ~(is_one | is_two);
			vector unsigned is_three = (vector unsigned) vec_cmpgt (2 * ix, vec_splat(consts, 1));
			vector unsigned is_threeb = (vector unsigned) vec_cmpgt (2 * iy, vec_splat(consts, 2));
			vector float xy = x + y;
			res = vec_sel(res, vasuint(xy), (is_three | is_threeb) & not_covered);
			not_covered &= ~(is_three | is_threeb);
			vector unsigned is_four = (vector unsigned) vec_cmpeq (2 * ix, vec_splat(consts, 3));
			res = vec_sel(res, vasuint(one), is_four & not_covered);
			not_covered &= ~is_four;
			vector unsigned is_fivea = (vector unsigned) vec_cmplt (2 * ix, vec_splat(consts, 3));
			vector unsigned is_fiveb = (vector unsigned) vec_cmpeq (iy & 0x80000000, zero);
		...
		vector unsigned is_ix = (vector unsigned) vec_cmpeq (zeroinfnan(ix), zero);
		if (!vec_all_eq(is_ix, zero)) {
			vector float x2 = x * x;
			vector unsigned is_onea = (vector unsigned) vec_cmpeq (ix & 0x80000000, zero);
			vector unsigned is_oneb = (vector unsigned) vec_cmpeq (checkint (iy), zero + 1);
			vector unsigned is_one = is_onea & is_oneb;
			x2 = vec_sel(x2, -x2, is_one);
#ifdef WANT_ERRNO
			vector unsigned is_divza = (vector unsigned) vec_cmpeq (2 * ix, zero);
			vector unsigned is_divzb = (vector unsigned) vec_cmpeq (iy & 0x80000000, zero);
			vector unsigned is_divz = is_divza & is_divzb;
			for (int m=0;!vec_all_eq(is_divz, zero) && m<4;m++) {
				if (is_divz[m] == 0)
					continue;
				res[m] = asuint(__math_divzerof (sign_bias[m]));
				res_mask[m] = 0xffffffff;
			}
#endif
			vector unsigned is_yneg = (vector unsigned) vec_cmpeq (iy & 0x80000000, zero); 
			vector float for_neg = 1 / x2;
			for_neg = vec_sel (x2, for_neg, is_yneg);
			res = vec_sel (res, vasuint(for_neg), is_one);
			res_mask |= is_one;
		}
		vector unsigned is_xneg = (vector unsigned) vec_cmpeq (ix & 0x80000000, zero);
		if (!vec_all_eq(is_xneg, zero)) {
			vector unsigned yint = checkint (iy);
			vector unsigned is_invalid = (vector unsigned) vec_cmpeq (yint, zero);
			if (!vec_all_eq(is_invalid, zero)) {
				for (int m=0;m<4;m++) {
					if (is_invalid[m] == 0)
						continue;
			...
			vector unsigned one = {1, 1, 1, 1};
			vector unsigned is_one = (vector unsigned) vec_cmpeq (yint, zero + 1);
			vector unsigned SIGN_BIASV = {SIGN_BIAS, SIGN_BIAS, SIGN_BIAS, SIGN_BIAS};
			sign_bias = vec_sel(sign_bias, SIGN_BIASV, is_one);
		}
		vector unsigned is_subnormal = (vector unsigned) vec_cmplt (ix, zero + 0x00800000);
		if (!vec_all_eq(is_subnormal, zero)) {
			vector unsigned subnormals = ix;
			subnormals = vasuint (x * 0x1p23f);
			subnormals &= zero + (unsigned)0x7ffffffff;
			subnormals -= 23 << 23;
			ix = vec_sel(ix, subnormals, is_subnormal);
		}
	...
	return exp2_inline(ylogxl, ylogxr, sign_bias);
}
